...
library(lhs)
library(IMIS) #default distribution is multivariate normal, but lets you do others (using copula)
...
# draws sample from Latin Hypercube design - same as RandomSearch code but wrapped in a function bc -
# - IMIS NEEDS a function called "sample.prior"
...
f_log_prior(v.params = sample.prior(100))

#this function evaluates the density of the prior
...
f_llik(v.params = sample.prior(10))

likelihood <- function(v.params){ 
  exp(f_llik(v.params)) 
}
likelihood(v.params = v.params.test)
likelihood(v.params = sample.prior(10))
...
f_log_post(v.params = sample.prior(10))
...
# obtian posterior values for draw
v.calib.post <- exp(f_log_post(m.calib.post))
...
plot(m.calib.post,
     ...
     col = scales::alpha("black", v.calib.alpha))
# add center of Gaussian components
points(fit.imis$center, col = "red", pch = 8)
...
v.calib.post.mode <- apply(m.calib.post, 2, 
                           function(x) as.numeric(modeest::mlv(x)[1]))
v.calib.post.mode
...
### Plot model-predicted output at mode vs targets ### MODE IS WHAT WE USE IF WE WANT 1 BEST FITTING SET
# but the mode is just representative - we would actually sample from the posteriors in our PSA and take the expectation
...
points(x = CRS.targets$Surv$Time, 
       ...
       pch = 8, col = "red")
legend("topright", 
       legend = c("Target", "Model-predicted output at mode"),
       col = c("black", "red"), pch = c(1, 8))
...
v.out.post.mean <- markov_crs(v.calib.post.mean)
