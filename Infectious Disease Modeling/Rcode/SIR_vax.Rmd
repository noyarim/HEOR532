...
Below, we defined the ode function with S,I,R compartment, with stratification by vaccine status. Non-vaccinated S moves to the vaccinated S at the rate of mu. In beta*S*I term, note that total I who can infect others is the sum of I_NotV and I_V. (1-alpha) indicates reduced susceptibility for vaccinated susceptibles and the higher alpha is, the lower the susceptibility would be.
```{r}
