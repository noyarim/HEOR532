...
---
  ...
  In this lab, we will examine growth of emerging epidemics, using regional data from the United States. The goals of this lab will be to illustrate the relationship between
growth rates and the reproductive number, and how the reproductive number can be estimated from early epidemic growth. We will illustrate alternative approaches to 
calculating the reproductive number. And we will show how variability in the reproductive nummber is important.
...
#this is how to manipulate data using tidyverse 'pipes'. 'filter' selects observations. 
#'mutate' creates new variables. we have to define the date variable as being a date with format Year-month-day.
...
It looks linear, plot a regression line to confirm 
...
Now that we have the growth rate, we can estimate $R_0$, assuming we know the latent and infectious periods. Let's assume that the latent period (L) 
is 3 days and the infectious period (D) is 4 days. Now, we can use the equation: $R_0 = (1 + \Lambda D)(1 + \Lambda L)$
...
This is a lesson in how projections based on fits to early data can go awry. The model fit very well for the first month, but the conditions changed. Let's 
look at cases on the log scale, and estimate the growth rate for March versus April.
...
The growth rate (slope) changed quite a bit, and it looks like it's still declining. Let's calculate the $R_0$ for April (to date) and compare it to what we estimated 
for the the first month. First we'll fit a regression of log cases against time to estimate the growth rate. Then we'll again use our equation for $R_0$ as above.
...
- Redo the above analyses, but instead for New York City (county "New York City"). 
  comment: see above code 
  
- What were the early growth rate and $R_0$ in New York City?
   R0 = 5.89 
...
- What would happen if we assumed a longer or shorter serial (generation) interval? 
  comment: if assume longer serial intervals, will have slower epidemic growth; pathogens with shorter serial interval lead to faster epidemic growth (i.e. has a higher growth rate).

- Try doubling the generation time (currently 5.8 days) or making it half as long. What does this do to $R_t$? Describe the change.#
  comment: see below graphs. Reach Rt < 1 faster when longer serial interval. 

```{r, message=FALSE, warning=FALSE}
mGT<-generation.time("gamma", c(12, 3)) #this is the generation interval, defined as a gamma distribution with a mean of 5.8 days
R0.res<-est.R0.TD(epid=case.inc$incid,GT=mGT)
...
```
```{r, message=FALSE, warning=FALSE}
mGT<-generation.time("gamma", c(3, 3)) #this is the generation interval, defined as a gamma distribution with a mean of 5.8 days
R0.res<-est.R0.TD(epid=case.inc$incid,GT=mGT)
