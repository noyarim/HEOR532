...
# HW 4
cPD <- 500 #cPD: cost of end of life drug (per-cycle cost)
...
# Added cPD for end of life drug
state_c_matrix <-
  matrix(c(cAsymp, cProg, 0,
           cAsymp + cDrug, cProg + cPD, 0),
         byrow = TRUE,
         nrow = n_treatments,
...
# time-homogeneous factoring in end-of-life treatment
# Without drug transitions stay the same

p_matrix <- array(data = c(1 - tpProg - tpDn, 0, 0,
                           tpProg, 1 - tpDcm - tpDn, 0,
                           tpDn, tpDcm + tpDn, 1,
                           1 - tpProg*(1-effect) - tpDn, 0, 0,
                           tpProg*(1-effect), 1 - tpDcm*effect - tpDn, 0,
                           tpDn, tpDcm*effect + tpDn, 1),
                  dim = c(n_states, n_states, n_treatments),
                  dimnames = list(from = s_names,
                                  to = s_names,
                                  t_names))
...
p_matrix_cycle <- function(p_matrix, age, cycle,
  ...
  p_matrix["Progressive_disease", "Dead", "with_drug"] <- tpDcm*effect + tpDn
  p_matrix["Progressive_disease", "Progressive_disease", "with_drug"] <- 1 - tpDcm*effect - tpDn
...
state_c_matrix <- function() {
  matrix(c(cAsymp(), cProg(), 0,            # without drug
           cAsymp() + cDrug(), cProg() + cPD, 0), # with drug
           byrow = TRUE,
           nrow = n_treatments,
           dimnames = list(t_names,
                           s_names))
...
# For HW Assignment
mean(costs[, "without_drug"])
mean(costs[, "with_drug"])
mean(qalys[, "without_drug"])
mean(qalys[, "with_drug"])
