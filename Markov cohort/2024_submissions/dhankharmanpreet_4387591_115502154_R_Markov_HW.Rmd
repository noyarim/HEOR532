---
title: "HEOR533_MarkovHW"
author: "Manpreet Dhankhar"
date: "2024-01-29"
...
1. Define parameters
```{r}
t_names <- c("without_drug", "with_drug", "EoL")#t_names : strategy/scenario label
...
cDrug <- 1000#cDrug: cost of drug, recurring for asymptomatic population
...
tpEoL <- 0.5#tpEoL: E0L care efficacy in decreasing excess mortality with progressive disease 
...
```{r}
# cost of staying in state, recurring
state_c_matrix <-
...
# qaly when staying in state, 3 for with drug and 3 for without drug
state_q_matrix <-
...
# this will be updated over and over again bc of changing age-specific mortality?
...
p_matrix <- array(data = c(1 - tpProg - tpDn, 0, 0,
                           ...
                           tpDn, tpDcm + tpDn, 1,
                           1 - tpProg*(1-effect) - tpDn, 0, 0,
                           tpProg*(1-effect), 1 - tpEoL*tpDcm - tpDn, 0,
                           tpDn, tpEoL*tpDcm - tpDn, 1),
...
pop["Asymptomatic_disease", cycle = 1, ] <- n_cohort # treatment level would usually come after comma, but not specified here, so it does it for both treatments
...
trans[, cycle = 1, ] <- 0 #impose 0 in the first cycle
```
...
total_costs <- setNames(c(NA, NA, NA), t_names) #sums for all three treatments
...
# incorporate age-specific mortality
...
p_matrix_cycle <- function(p_matrix, age, cycle,
  ...
  tpDn_lookup <-
    c("(34,44]" = 0.0017,
      ...
      "(64,74]" = 0.0379,
      "(74,84]" = 0.0912,
      "(84,100]" = 0.1958) # mortality depending on age
  ...
  # Matrix containing transition probabilities for EoL care
  p_matrix["Asymptomatic_disease", "Progressive_disease", "EoL"] <- tpProg*(1 - effect)*cycle
  p_matrix["Asymptomatic_disease", "Dead", "EoL"] <- tpDn
  p_matrix["Asymptomatic_disease", "Asymptomatic_disease", "EoL"] <-
    1 - tpProg*(1 - effect)*cycle - tpDn
  p_matrix["Progressive_disease", "Dead", "EoL"] <- tpEoL*tpDcm + tpDn
  p_matrix["Progressive_disease", "Progressive_disease", "EoL"] <- 1 - tpEoL*tpDcm - tpDn
  p_matrix["Dead", "Dead", "EoL"] <- 1
...
8. Simulate a cohort for 'no drug' and 'drug' amd 'Eol care' scenario
...
c_incr_2vs1 <- total_costs["with_drug"] - total_costs["without_drug"]
c_incr_3vs1 <- total_costs["EoL"] - total_costs["without_drug"]
c_incr_3vs2 <- total_costs["EoL"] - total_costs["with_drug"]

q_incr_2vs1 <- total_QALYs["with_drug"] - total_QALYs["without_drug"]
q_incr_3vs1 <- total_QALYs["EoL"] - total_QALYs["without_drug"]
q_incr_3vs2 <- total_QALYs["EoL"] - total_QALYs["with_drug"]
...
ICER_2vs1 <- c_incr_2vs1/q_incr_2vs1
ICER_3vs1 <- c_incr_3vs1/q_incr_3vs1
ICER_3vs2 <- c_incr_3vs2/q_incr_3vs2

```

11a. Plot scenario 2 vs 1: with_drug vs without_drug
```{r}

wtp <- 20000
plot(x = q_incr_2vs1/n_cohort, y = c_incr_2vs1/n_cohort,
     xlim = c(0, 2),
     ylim = c(0, 15e3),
     pch = 16, cex = 1.5,
...
png("figures_ceplane_point.png", width = 4, height = 4, units = "in", res = 640)
plot(x = q_incr_2vs1/n_cohort, y = c_incr_2vs1/n_cohort,
     xlim = c(0, 2),
     ylim = c(0, 15e3),
     pch = 16, cex = 1.5,
...
11b. Plot scenario 3 vs 1: EoL vs without_drug
```{r}

wtp <- 20000
plot(x = q_incr_3vs1/n_cohort, y = c_incr_3vs1/n_cohort,
     xlim = c(0, 2),
     ylim = c(0, 15e3),
     pch = 16, cex = 1.5,
     xlab = "QALY difference",
...
11c. Plot scenario 3 vs 2: EoL vs with_drug
```{r}

wtp <- 20000
plot(x = q_incr_3vs2/n_cohort, y = c_incr_3vs2/n_cohort,
     xlim = c(0, 2),
     ylim = c(0, 15e3),
     pch = 16, cex = 1.5,
     xlab = "QALY difference",
