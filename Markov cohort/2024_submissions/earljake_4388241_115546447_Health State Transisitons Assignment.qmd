---
title: "Health State Transition Assignment"
author: "Jake Earl"
format: html
...
1. Define parameters
```{r}
t_names <- c("without_treatment", "with_treatment_without_EOLc", "with_treatment_with_EOLc")#t_names : strategy/scenario label EOLc = End of Life care
...
tpDn <- 0.0379  #tpDn: baseline mortality <- mortality 0.0379  # over 65 year old

effect <- 0.5#effect: drug efficacy in decreasing the risk of progressing from asymptomatic to progressive disease AND decreasing excess mortality with progressive disease
...
# time-homogeneous - need an array for the additional dimensions
p_matrix <- array(data = c(1 - tpProg - tpDn, 0, 0,
                           ...
                           1 - tpProg*(1-effect) - tpDn, 0, 0,
                           tpProg*(1-effect), 1 - tpDcm*(1-effect) - tpDn, 0,
                           tpDn, tpDcm*(1-effect) + tpDn, 1),
                  
                  dim = c(n_states, n_states, n_treatments), #rows, columns, dimensions
                  dimnames = list(from = s_names,
                                  to = s_names,
                                  t_names))
p_matrix
...
# state populations - need an array here as well for the additional dimension
pop <- array(data = NA, #don't know the value yet
...
```{r}
# _arrived_ state populations - Need an array again
trans <- array(data = NA,
               dim = c(n_states, n_cycles, n_treatments),
               dimnames = list(state = s_names,
                               cycle = NULL,
...
trans[, cycle = 1, ] <- 0 #no cost in the first cycle
trans
```
...
p_matrix_cycle <- function(p_matrix, age, cycle, #these are for deterministic results
                           tpProg = 0.01, #the following three lines are for PSA
                           tpDcm = 0.15,
                           effect = 0.5) {
  tpDn_lookup <-
    c("(34,44]" = 0.0017,
  ...
  p_matrix["Asymptomatic_disease", "Progressive_disease", "without_treatment"] <- tpProg*cycle
  p_matrix["Asymptomatic_disease", "Dead", "without_treatment"] <- tpDn
  p_matrix["Asymptomatic_disease", "Asymptomatic_disease", "without_treatment"] <- 1 - tpProg*cycle - tpDn
  p_matrix["Progressive_disease", "Dead", "without_treatment"] <- tpDcm + tpDn
  p_matrix["Progressive_disease", "Progressive_disease", "without_treatment"] <- 1 - tpDcm - tpDn
  p_matrix["Dead", "Dead", "without_treatment"] <- 1
  
  # Matrix containing transition probabilities for with_drug
  p_matrix["Asymptomatic_disease", "Progressive_disease", "with_treatment_without_EOLc"] <- tpProg*(1 - effect)*cycle
  p_matrix["Asymptomatic_disease", "Dead", "with_treatment_without_EOLc"] <- tpDn
  p_matrix["Asymptomatic_disease", "Asymptomatic_disease", "with_treatment_without_EOLc"] <-
    1 - tpProg*(1 - effect)*cycle - tpDn
  p_matrix["Progressive_disease", "Dead", "with_treatment_without_EOLc"] <- tpDcm + tpDn
  p_matrix["Progressive_disease", "Progressive_disease", "with_treatment_without_EOLc"] <- 1 - tpDcm - tpDn
  p_matrix["Dead", "Dead", "with_treatment_without_EOLc"] <- 1

  # Matrix containing transition probabilities for with_drug_with_EOLc
  p_matrix["Asymptomatic_disease", "Progressive_disease", "with_treatment_with_EOLc"] <- tpProg*(1 - effect)*cycle
  p_matrix["Asymptomatic_disease", "Dead", "with_treatment_with_EOLc"] <- tpDn
  p_matrix["Asymptomatic_disease", "Asymptomatic_disease", "with_treatment_with_EOLc"] <-
    1 - tpProg*(1 - effect)*cycle - tpDn
  p_matrix["Progressive_disease", "Dead", "with_treatment_with_EOLc"] <- tpDcm*(1 - effect)*cycle + tpDn
  p_matrix["Progressive_disease", "Progressive_disease", "with_treatment_with_EOLc"] <- 1 - tpDcm*(1 - effect)*cycle - tpDn
  p_matrix["Dead", "Dead", "with_treatment_with_EOLc"] <- 1
  
  return(p_matrix) #updates p_matrix
}
```
...
for (i in 1:n_treatments) { # outer loop: over strategies
  ...
  for (j in 2:n_cycles) { # inner loop: over cycles
    ...
    p_matrix <- p_matrix_cycle(p_matrix, age, j - 1, effect = effect) #effect = effect is added for PSA!
  ...
  cycle_trans_costs[i, ] <-
    (c(1,1,1) %*% trans[, , treatment = i]) * 1/(1 + cDr)^(1:n_cycles - 2) # dot product with c(1,1,1,1) sums transitional cost across states and generate per-cycle total transitional cost
  # per-cycle cost is the summ of state cost and transitional cost
  cycle_costs[i, ] <- cycle_state_costs[i, ] + cycle_trans_costs[i, ]
  
  # life expectancy
  LE[i, ] <- c(1,1,0) %*% pop[, , treatment = i] #only want to count the non-death state
  ...
  total_costs[i] <- sum(cycle_costs[treatment = i, -1]) #excluded the first cycle to start at time 0
  total_QALYs[i] <- sum(cycle_QALYs[treatment = i, -1]) #excluded the first cycle to start at time 0
}
```
...
c_incr <- total_costs["with_treatment_without_EOLc"] - total_costs["without_treatment"]
q_incr <- total_QALYs["with_treatment_without_EOLc"] - total_QALYs["without_treatment"]
c2_incr <- total_costs["with_treatment_with_EOLc"] - total_costs["without_treatment"]
q2_incr <- total_QALYs["with_treatment_with_EOLc"] - total_QALYs["without_treatment"]
```
...
png("/Users/jake/Desktop/R_Files/UW_Classwork/Q2_Classes/HEOR/Pictures (Test)/ceplane_point.png", width = 4, height = 4, units = "in", res = 640)
plot(x = q_incr/n_cohort, y = c_incr/n_cohort,
     xlim = c(0, 2),
     ylim = c(0, 15e3),
     pch = 16, cex = 1.5,
...
wtp <- 20000
data <- data.frame(
  Type = c("Treatment", "Treatment & EOLc"),
  QALY_Diff = c(q_incr, q2_incr)/n_cohort,
  Cost_Diff = c(c_incr, c2_incr)/n_cohort
)


p <- ggplot(data, aes(x = QALY_Diff, y = Cost_Diff, color = Type)) +
  geom_point(pch = 16, size = 3) +
  xlim(0, 1) +
  ylim(0, 15000) +
  xlab("QALY difference") +
  ylab(paste0("Cost difference (", enc2utf8("\u00A3"), ")")) +
  geom_abline(intercept = 0, slope = wtp) +
  theme_minimal() +
  ggtitle("Treatment with EOLc and a Willingness to Pay (WTP) of 2000")

p
ggsave("/Users/jake/Desktop/ICER.png", plot = p, width = 10, height = 8, dpi = 300)
```
...
2. Random sampling of parameters
```{r}
# replace point values with functions to random sample - can set.seed if you want
cAsymp <- function() rnorm(1, 500, 127.55)#one number, mean, SD
...
png("/Users/jake/Desktop/R_Files/UW_Classwork/Q2_Classes/HEOR/Pictures (Test)/ceplane_psa.png", width = 4, height = 4, units = "in", res = 640)
plot(x = q_incr_psa/n_cohort, y = c_incr_psa/n_cohort,
     xlim = c(0, 2),
     ylim = c(0, 15e3),
     pch = 16, cex = 1.2,
