...
## Report the ICER estimate and standard error of mean cost and QALY.
...
#rn_sex <- runif(n_pop)
#v_sex <- as.numeric(rn_sex < dSex['female']) # 1=female, 0=male another way of sampling sex variable
table(v_sex)
...
MicroSim <- function(m_health, n_pop, t_end, v_state, d.c, d.e, TR.out = TRUE, TS.out = TRUE, Trt = FALSE, seed = 1) {
  ...
  if (TR.out == TRUE) { # create a trace from the individual trajectories, similar to markov trace
...
# overall prevalence (same prevalence between trt and no_trt)
ggplot(data=dt_prev)+
